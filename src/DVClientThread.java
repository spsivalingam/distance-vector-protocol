    ...
    public void run() { 
        while(true) {
			//try {
				...
				count++;
				System.out.println("\nAt Host " + hostName + ", ## Output Number " + count + "\n");
			//}
			...
			try {							   
				...
				if(count == 1) {
					readHost(file);//input file with ports n costs read n inserted into a hastable
					...
					// for(String neighbor : neighbors.keySet()) {
						...
						clientPacket = new DatagramPacket(bufSent, bufSent.length, address, Integer.parseInt(neighbors.get(n)));
...
*/
	...
	private String DVToRT(Hashtable<String, Vector<DistanceVector>> dvList) {
		...
		for(Vector<DistanceVector> v : dvList.values()) {
			...
			int ind = findMinIndex(costs);
			source = v.get(ind).getSource();
			dest = v.get(ind).getDest();
			via = v.get(ind).getVia();
			min = v.get(ind).getCost();
			
			res +=  source+";" + dest+";"+ via+";"+ min + "\n";
		}
	...
	private String simplePrint(String input) {
		...
		for(int i=0; i<lines.length; i++) {
			...
			res += "shortest path: " + start + " to " + dest + " via " + via + space + array[3] + "\n";			
